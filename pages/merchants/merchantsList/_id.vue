        ...
        <div class="shop-msg clearfix">
          <ul class="shop-left left">
            <li v-if="projectList.length!=0" v-for="item in projectList" @click="goShopDetail(item)">
              ...
              <div class="price">
                <div><span>{{item.price}}</span>元/天/㎡</div>
                <p>(均价)</p>
              </div>
            </li>
          ...
          <div class="shop-right right">
            <h3>火热项目</h3>
            <ul>
              <li @click="goShopDetail(item)" v-for="item in hotProjectList">
                <div class="img">
                  <img :src="item.pictures" alt="" :onerror="defaultImg">
                  <p>{{item.address}}</p>
                </div>
    ...
    methods: {
      ...
      lists(){//列表初始化
        ...
        axios({
          ...
          this.hotProjectList = res.data.hotProjectList;
      ...
      regionFun(e,item){ //区域
        // this.$router.push({name:"merchants-merchantsList-id",params:{id:item.id}})
      ...
      goShopDetail(item){//跳转到详情页面
        ...
        window.open("/merchants/merchantsDetail/"+this.projectType+"/"+item.projectId+"/"+item.houseType);
      }
    }
  }
</script>
